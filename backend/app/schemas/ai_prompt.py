...
class PromptBase(BaseModel):
    ...
    # <-- ИЗМЕНЕНО 'content' на 'prompt_text'
...
class PromptCreate(PromptBase):
    is_active: bool = Field(
        True, description="Is this prompt currently active?")
...
class PromptUpdate(BaseModel):
    ...
    # # Раскомментируйте, если имя можно обновлять
    # <-- ИЗМЕНЕНО 'content' на 'prompt_text'
...
class PromptOut(PromptBase):
    ...
    profile_id: str = Field(...,
                            description="Associated Profile ID (UUID as string)")
    is_active: bool = Field(...,
                            description="Is this prompt currently active?")
    # Наследует 'name' и 'prompt_text'
...
class PromptPreviewRequest(BaseModel):
    description: str = Field(
        ..., description="Job description to use for preview"
    )


# Схема для ответа превью
