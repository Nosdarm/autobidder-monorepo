...
def get_user_template(user_id: str):
    ...
    if not os.path.exists(path):
        return {
            "prompt": "Ты опытный специалист. Напиши отклик на вакансию профессионально.",
            "template": "Здравствуйте! Я увидел вашу задачу: {title}. У меня есть соответствующий опыт и я хотел бы обсудить детали."
        }
...
def api_update_template(user_id: str, data: dict):
    if "prompt" not in data or "template" not in data:
        raise HTTPException(status_code=400, detail="Missing 'prompt' or 'template'")
